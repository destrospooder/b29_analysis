...
class Airfoil:
    def __init__(self, chord_length, max_thickness, max_thickness_loc, max_camber, max_camber_loc, name = None):
    ...
    def process_coords(self, filename):
        ...
        self.bottom_curve = (CubicSpline(self.x_coords[19:], self.y_coords[19:]))(self.xs)
    ...
    def thin_airfoil_theory(self, alpha, load = '', save = ''):
        ...
        self.zbar = (np.polyfit(self.xs, self.mean_chord_line, 7) @ np.vstack([x**7, x**6, x**5, x**4, x**3, x**2, x, 1]))[0]

        dzbar_dx = (sp.diff(self.zbar, x)).subs(x, (self.chord_length / 2) * (1 - sp.cos(phi)))

        if (load == ''):
            self.A0_minus_alpha = (-1 / np.pi) * sp.integrate(dzbar_dx, (phi, 0, np.pi))
            print('A0 calculated')
            self.A1 = (2 / np.pi) * sp.integrate(dzbar_dx * sp.cos(phi), (phi, 0, np.pi))
            print('A1 calculated')
            self.A2 = (2 / np.pi) * sp.integrate(dzbar_dx * sp.cos(2 * phi), (phi, 0, np.pi))
            print('A2 calculated')
            if (save != ''):
                print('Saving to data/fourier_coeffs/' + save)
                np.savez('data/fourier_coeffs/' + save, A0_minus_alpha = self.A0_minus_alpha, A1 = self.A1, A2 = self.A2)
            ...
            self.A0_minus_alpha = data['A0_minus_alpha']
            self.A1 = data['A1']
            self.A2 = data['A2']    
...
class Aircraft:
    def __init__(self, max_takeoff_weight, cruise_speed):
...
plane = Aircraft(133500 * 4.44822, 220 * 0.44704)

density = 0.377 # kg/m^3
